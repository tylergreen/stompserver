...
class TestClient < Test::Unit::TestCase
  ...
  def test_no_message
    received = nil
    @client.subscribe("/queue/lonely") {|msg| received = msg}
    sleep 1
    assert_equal received, nil
  end

  def test_timedout_message
    dest = "/queue/justone"
    @client.publish dest, 'a message that no one will get'
    sleep 21  # whatever timeout is
    received = nil
    @client.subscribe(dest) {|msg| puts 'should not happen!' ; received = msg}
    sleep 1
    assert_equal received, nil
end
